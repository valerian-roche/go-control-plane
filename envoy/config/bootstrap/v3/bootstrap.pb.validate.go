...
func (m *Bootstrap_DynamicResources) validate(all bool) error {
	...
	{
		sorted_keys := make([]string, len(m.GetAdditionalAdsConfigs()))
		i := 0
		for key := range m.GetAdditionalAdsConfigs() {
			sorted_keys[i] = key
			i++
		}
		sort.Slice(sorted_keys, func(i, j int) bool { return sorted_keys[i] < sorted_keys[j] })
		for _, key := range sorted_keys {
			val := m.GetAdditionalAdsConfigs()[key]
			_ = val

			// no validation rules for AdditionalAdsConfigs[key]
			...
			if all {
				...
				case interface{ ValidateAll() error }:
					if err := v.ValidateAll(); err != nil {
						errors = append(errors, Bootstrap_DynamicResourcesValidationError{
							field:  fmt.Sprintf("AdditionalAdsConfigs[%v]", key),
							reason: "embedded message failed validation",
							cause:  err,
						})
					}
