...
func TestLinearSotwWatches(t *testing.T) {
	...
	t.Run("watches are properly removed from all objects", func(t *testing.T) {
		...
		sotwState := stream.NewSubscriptionState(false, nil)
		w := make(chan Response, 1)
		_, err = cache.CreateWatch(&Request{ResourceNames: []string{"a", "b", "c"}, TypeUrl: testType, VersionInfo: cache.getVersion()}, &sotwState, w)
		require.NoError(t, err)
		mustBlock(t, w)
		...
		// c no longer watched
		w = make(chan Response, 1)
		_, err = cache.CreateWatch(&Request{ResourceNames: []string{"a", "b"}, TypeUrl: testType, VersionInfo: cache.getVersion()}, &sotwState, w)
		require.NoError(t, err)
		mustBlock(t, w)
		checkVersionMapNotSet(t, cache)
		...
		w = make(chan Response, 1)
		_, err = cache.CreateWatch(&Request{ResourceNames: []string{"c"}, TypeUrl: testType, VersionInfo: cache.getVersion()}, &sotwState, w)
		require.NoError(t, err)
		mustBlock(t, w)
		checkVersionMapNotSet(t, cache)
